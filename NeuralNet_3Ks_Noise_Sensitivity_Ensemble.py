...
EXPERIMENT_NAME = f"Ensemble_{BASE_EXPERIMENT_NAME}"

BASE_RESULTS_DIR = Path("Results_NN")
SAVED_WEIGHTS_ROOT = Path("saved_weights")

# Main output root requested by you for the ensemble version:
# Results_NN/O2_novib/Ensemble_(Individual_Noise)
INDIVIDUAL_NOISE_RESULTS_ROOT = BASE_RESULTS_DIR / SCHEME / EXPERIMENT_NAME
...
#
# IMPORTANT:
#   - 0% is handled as one shared clean baseline per species/architecture/seed.
...
# In this ensemble version, all these seeds are evaluated on the same noisy input
# realization and their scaled K predictions are averaged before the MSE is computed.
...
# Each line is one noisy subset, and every line starts from the shared 0% ensemble baseline.
# --------------------------------------------------------------------------------------
...
# ======================================================================================
# SCALING AND ENSEMBLE INDIVIDUAL-NOISE HELPERS
# ======================================================================================
...
def noise_rng_seed(noise_std, noise_repeat, noisy_subset_id):
    """Noise RNG seed independent of NN seed.

    Every ensemble member must see the same noisy input realization. Therefore the
    neural-network training seed is deliberately excluded from this seed.
    """
...
def result_key_from_values(noisy_subset_id, hidden_size_text, noise_std, noise_repeat):
    return (
        str(noisy_subset_id),
...
def load_existing_subset_results(results_root, kept_species, noisy_subset_id):
    ...
    if missing:
        print(
            f"Warning: existing results at {path} are missing columns {missing}. "
...
def aggregate_individual_noise_results(df):
    ...
    if missing_group_cols:
        raise ValueError(
            "Cannot aggregate ensemble individual-noise results. Missing columns: "
            + ", ".join(missing_group_cols)
        )

    metric_cols = [col for col in df.columns if col.endswith("_scaled")]
    if not metric_cols:
        raise ValueError("Cannot aggregate ensemble individual-noise results. No '*_scaled' metric columns were found.")
    ...
    if "num_newly_trained_models" in df.columns:
        agg_dict["num_newly_trained_models"] = ["mean", "min", "max"]
    if "all_models_reused_saved_weights" in df.columns:
        work_df = df.copy()
...
def append_current_results_to_subset_folders(results_root, current_results):
    ...
    if current_df.empty:
        print("No new ensemble evaluations were produced. Existing subset results are preserved.")
        return
...
def rebuild_global_files_from_subset_folders(results_root):
    ...
    if not all_subset_dfs:
        print(
            "No valid ensemble individual-noise subset folders found. "
    ...
    full_df.to_csv(results_root / "fullrun_ensemble_individual_noise_results.csv", index=False)

    full_agg = aggregate_individual_noise_results(full_df)
    full_agg.to_csv(results_root / "fullrun_ensemble_individual_noise_aggregate_summary.csv", index=False)
    ...
    for species_config_name, species_df in full_df.groupby("species_config_name", sort=False):
        ...
        species_df.to_csv(species_root / "ensemble_individual_noise_results.csv", index=False)
        species_agg = aggregate_individual_noise_results(species_df)
        species_agg.to_csv(species_root / "ensemble_individual_noise_aggregate_summary.csv", index=False)

    rebuild_info = {
        "rebuild_policy": (
            "fullrun files rebuilt from valid first-level species-combination folders "
            "and valid second-level ensemble individual-noise subset folders containing noise_results.csv"
        ),
    ...
    print("Rebuilt global ensemble individual-noise files from subset folders.")
...
def plan_tasks_for_species(results_root, kept_species):
    ...
    # Shared clean baseline: one ensemble evaluation per architecture.
...
def save_run_info(results_root, task_plan, total_requested, total_existing, total_to_run):
    ...
    run_info = {
        ...
        "cache_policy": "load compatible saved_weights model for every ensemble seed; otherwise train and save before evaluation",
        "append_policy": (
            "new ensemble rows are appended to each subset noise_results.csv; duplicate rows for the same "
            "species/noisy_subset/architecture/noise/repeat are de-duplicated keeping the newest row"
        ),
    ...
    save_json(results_root / "ensemble_individual_noise_run_info.json", run_info)
...
def load_aggregate_results_for_plotting(results_root):
    results_root = Path(results_root)
    aggregate_path = results_root / "fullrun_ensemble_individual_noise_aggregate_summary.csv"
    ...
    if df is None or df.empty:
        raise FileNotFoundError(
            f"Could not find or rebuild aggregate ensemble individual-noise results in {results_root}."
        )
...
def save_individual_noise_sensitivity_plots(results_root=None):
    ...
    for species_config_name, species_df in df.groupby("species_config_name", sort=False):
        ...
        for hidden_size, arch_df in species_df.groupby("hidden_size", sort=False):
            ...
            if baseline_df.empty:
                print(
                    f"Warning: no clean ensemble baseline found for {species_config_name}, "
            ...
            output_base = species_plot_root / f"architecture_{arch_to_file_token(hidden_size)}__ensemble_individual_noise_sensitivity"
    ...
    manifest = {
        ...
        "clean_baseline_policy": "shared 0% ensemble point plotted at the start of every noisy-subset line",
        "num_created_files": int(len(created_files)),
        "created_files": created_files,
    }
    save_json(results_root / "Plots" / "plot_manifest.json", manifest)

    print(f"Saved ensemble individual-noise sensitivity plots to: {plots_root}")
...
def run_individual_noise_sensitivity():
    ...
    print("Requested ensemble evaluations:", total_requested)
    print("Already existing requested ensemble evaluations:", total_existing)
    print("New ensemble evaluations to run:", total_to_run)
    ...
    if REBUILD_GLOBAL_FILES_FROM_ALL_SUBSET_FOLDERS:
        ...
        full_df.to_csv(INDIVIDUAL_NOISE_RESULTS_ROOT / "fullrun_ensemble_individual_noise_results.csv", index=False)
        full_agg.to_csv(INDIVIDUAL_NOISE_RESULTS_ROOT / "fullrun_ensemble_individual_noise_aggregate_summary.csv", index=False)

    print(f"\nSaved ensemble individual-noise results to: {INDIVIDUAL_NOISE_RESULTS_ROOT}")
    ...
    if SAVE_PLOTS_AFTER_RUN:
        try:
            ...
            print(f"WARNING: Ensemble individual-noise plotting failed after results were saved: {exc}")
