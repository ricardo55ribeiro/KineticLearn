...
SCHEME = "O2_novib"
EXPERIMENT_NAME = "Noise_Error_Rankings"
...
def result_key_from_values(hidden_size_text, seed, noise_std, noise_repeat):
    return (
...
def load_existing_species_results(noise_results_root, kept_species):
    ...
    required = ["hidden_size", "seed", "noise_std", "noise_repeat"]
...
def deduplicate_results(df):
    ...
    required = ["scheme", "experiment_name", "species_config_name", "hidden_size", "seed", "noise_repeat", "noise_std"]
    missing = [col for col in required if col not in df.columns]
    if missing:
        print(
...
def aggregate_noise_results(df):
    ...
    if not metric_cols:
        raise ValueError("Cannot aggregate noise results. No '*_scaled' metric columns were found.")
                    ...
                    tasks.append(
                        {
                            ...
                            "noise_std": float(noise_std),
                            "noise_repeat": int(noise_repeat),
                            "key": result_key_from_values(hidden_size_text, seed, noise_std, noise_repeat),
                        }
                    )
    return tasks
...
def save_ranking_outputs(noise_results_root, aggregate_df):
    ...
    txt_lines = [
        "Noise/architecture ranking build information",
        "",
...
def add_scatter_point_id_labels(ax, plot_df):
    for _, row in plot_df.iterrows():
        ax.annotate(
            ...
            xytext=(3, 3),
            fontsize=SCATTER_POINT_LABEL_FONT_SIZE,
            ha="left",
            va="bottom",
...
def save_run_info(noise_results_root, total_requested, total_existing, total_to_run):
    run_info = {
        ...
        "append_policy": (
            "new rows are appended to each species noise_results.csv; duplicate rows for the same "
